...
func NamedInSql(query string, arg map[string]interface{}, db *sqlx.DB) (string, []interface{}, error) {
...
func (h *handlers) GetGrades(c echo.Context) error {
	...
	for _, course := range registeredCourses {
		...
		query = "SELECT `submissions`.`score`, class_id FROM `submissions` WHERE `user_id` = :userID AND `class_id` IN (:classIDs)"
		query, args, err = NamedInSql(query, input, h.DB)
		if err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
